...
def translate_history_table_name(english_name):
    """将英文表名翻译为中文显示名"""
    return TABLE_TRANSLATIONS.get(english_name, english_name)

def translate_history_field_name(english_field):
    """将英文字段名翻译为中文显示名"""
...
def translate_dataframe_columns(df):
    ...
    for col in df.columns:
        chinese_columns[col] = translate_history_field_name(col)
...
def translate_table_options(table_list):
    ...
    for table in table_list:
        if isinstance(table, dict):
            ...
            chinese_label = translate_history_table_name(table['value'])
            ...
            # 如果是字符串格式
            translated_options.append({
                'label': translate_history_table_name(table),
                'value': table
            })
